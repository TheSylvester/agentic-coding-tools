---
name: fusionthink
description: Run a multi-vendor adversarial review by dispatching a Claude reviewer and a Codex reviewer over one identical brief, then skeptically verifying every claim against the real code, pushing back on weak findings, and settling disputes cross-vendor. Use for fast two-vendor analysis of a plan, diff, set of files, or design question, or to converge on an artifact by fixing and re-reviewing until both reviewers agree it is ready.
license: MIT
---

# Fusionthink

Two independent reviewers from different vendors read the same brief. You
verify what they say against the actual code, push back where it does not hold
up, and let each vendor settle the other's disputes.

**Requires the sibling skills `claude-agent` and `codex-agent`, plus both
vendor CLIs (`claude` and `codex`) installed and authenticated.** Without both
lanes this skill has no reason to exist — use a single reviewer instead.
...
Write the prompt to one temp file with a **unique stub** so concurrent runs
cannot collide — for example `fusionthink-<timestamp>-<random>.md` in the
system temp directory. Both agents read that same file.

Launch both wrappers **in the same message**, both in the background, using
`PROMPT_FILE` so the brief reaches each reviewer byte-exact on stdin:

- **claude**: `PROMPT_FILE=<brief> node "~/.claude/skills/claude-agent/scripts/claude-agent.mjs"`
- **codex**: `PROMPT_FILE=<brief> node "~/.claude/skills/codex-agent/scripts/codex-agent.mjs"`

Both get the **identical prompt**. Both reviewers are full independent agents
over the same brief.

## Wait for BOTH agents, then collect output
...
Trust the background-task notification; do not poll by default. If you
genuinely need a liveness check on a long run, launch the Claude reviewer with
`--monitor` (live phase and `STALLED!!` lines on stderr) or watch the codex
`[output_file:]` — a growing file or an advancing phase means it is working,
long-stale means investigate.

### Collect output

Each wrapper prints `[output_file: <path>]` on stderr and ends its reply with
`[session_id: <uuid>]`. The `[output_file:]` `.log` holds the clean reply plus
the `[session_id:]` line in every mode.

For each reviewer, read the **exact `[output_file:]` path** the wrapper printed
and extract its final analysis. Do **not** glob for the newest
`codex-agent-*.log` — the filenames carry no run stub, so "newest" can belong
to a different concurrent run. **Capture each reviewer's `[session_id:]`** for
the resume operations below. If a `[session_id:]` is absent, treat that
vendor's resume as unavailable and say so rather than guessing an id.
...
- **claude**: `node "~/.claude/skills/claude-agent/scripts/claude-agent.mjs" --resume <session-id> "You were investigating <topic>. Synthesize your findings into a concrete analysis. What did you find? What's the root cause? What's the fix?"`
- **codex**: `node "~/.claude/skills/codex-agent/scripts/codex-agent.mjs" --resume <session-id> "You were investigating <topic>. Synthesize your findings into a concrete analysis. What did you find? What's the root cause? What's the fix?"`
...
- **claude**: `node "~/.claude/skills/claude-agent/scripts/claude-agent.mjs" --resume <session-id> "pushback message"`
- **codex**: `node "~/.claude/skills/codex-agent/scripts/codex-agent.mjs" --resume <session-id> "pushback message"`
...
were addressed correctly. Use the session IDs captured from the `[session_id:]`
lines:

- **claude**: `PROMPT_FILE=<round-N brief> node "~/.claude/skills/claude-agent/scripts/claude-agent.mjs" --resume <claude-session-id>`
- **codex**: `PROMPT_FILE=<round-N brief> node "~/.claude/skills/codex-agent/scripts/codex-agent.mjs" --resume <codex-session-id>`
...
- **Either says NOT READY** → run the same skeptical verification, pushback,
  and dispute-settlement on the new claims; apply only confirmed fixes; then
  re-dispatch round N+1.
