...
Create a plan file at `.ai-reference/plans/<feature-name>.md` with two sections: `## Agreed` and `## Open / Not Yet Agreed`. Infer the feature name from context, or ask.
...
As new questions surface during discussion, add them to Open. After writing to the file, briefly remind the user what's still open. The spec is done when Open is empty or deferred.
